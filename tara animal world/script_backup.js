class AnimalGeographyGame {
    ...
    initializeDropZones() {
        ...
        // Handle floating ocean zone if it exists
        const floatingOceanZone = document.querySelector('.floating-ocean-zone');
        if (floatingOceanZone) {
            floatingOceanZone.addEventListener('dragover', (e) => {
                e.preventDefault();
                floatingOceanZone.style.transform = 'translateY(-8px) scale(1.05)';
            });

            floatingOceanZone.addEventListener('dragleave', (e) => {
                floatingOceanZone.style.transform = 'translateY(-5px)';
            });            floatingOceanZone.addEventListener('drop', (e) => {
                e.preventDefault();
                floatingOceanZone.style.transform = 'translateY(-5px)';
                
                if (this.draggedAnimal) {
                    this.handleDrop('ocean', this.draggedAnimal);
                }
            });
        }
    }
